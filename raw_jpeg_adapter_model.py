...
Net architecture for raw image pre-processing (prior to JPEG compression) & post-processing (after JPEG decompression).
"""
...
from img_utils import tensor_to_img, imwrite, img_to_tensor
import zlib
import base64
import os
...
class JPEGAdapter(nn.Module):
  ...
  def encode_params(params: Dict[str, Union[torch.Tensor, None]]) -> str:
    ...
    if params['gamma_map'] is not None:
      gamma_map = tensor_to_img(params['gamma_map'].squeeze(0)).astype(JPEG_ADAPT_OUT_DTYPE)
    ...
    if params['scale_dct'] is not None:
      scale_dct = params['scale_dct'].detach().cpu().numpy().astype(JPEG_ADAPT_OUT_DTYPE)
    ...
    if params['lut'] is not None:
      lut = params['lut'].detach().cpu().numpy().astype(JPEG_ADAPT_OUT_DTYPE)
  ...
  def decode_params(comment: str, dtype: Optional[torch.dtype]=torch.float32,
    ...
    if lut_shape_str != '$':
      ...
      lut_shape = None
    gamma_map = decode_field(gamma_encoded, map_shape, JPEG_ADAPT_OUT_DTYPE) if gamma_encoded != '$' else None
    scale_dct = decode_field(scale_dct_encoded, (3, 1, 8, 8), JPEG_ADAPT_OUT_DTYPE)

    lut = decode_field(lut_encoded, lut_shape, JPEG_ADAPT_OUT_DTYPE) if lut_encoded != '$' else None
